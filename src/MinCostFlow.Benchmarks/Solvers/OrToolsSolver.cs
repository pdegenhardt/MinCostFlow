...
/// </summary>
public class OrToolsSolver(IGraph graph)
{
    private readonly IGraph _graph = graph;
    private readonly GoogleMinCostFlow _solver = new GoogleMinCostFlow();
    private readonly Dictionary<int, long> _nodeSupplies = [];
    private readonly Dictionary<int, (long lower, long upper)> _arcBounds = [];
    private readonly Dictionary<int, long> _arcCosts = [];
    private readonly Dictionary<(int from, int to), int> _arcLookup = [];
    private readonly Dictionary<int, (long lower, long cost)> _arcLowerBoundCosts = [];
    ...
    public SupplyType GetSupplyType()
    {
        ...
        if (totalSupply > 0)
        {
            return SupplyType.Geq;
        }

        if (totalSupply < 0)
        {
            return SupplyType.Leq;
        }
    ...
    private static SolverStatus ConvertStatus(GoogleMinCostFlow.Status orToolsStatus)
    {
        return orToolsStatus switch
        {
            GoogleMinCostFlow.Status.OPTIMAL => SolverStatus.Optimal,
            GoogleMinCostFlow.Status.INFEASIBLE => SolverStatus.Infeasible,
            GoogleMinCostFlow.Status.UNBALANCED => SolverStatus.Infeasible,// Unbalanced is a type of infeasibility
            _ => SolverStatus.NotSolved,
        };
    }
}