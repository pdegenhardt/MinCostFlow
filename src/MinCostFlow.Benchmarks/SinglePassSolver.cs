...
namespace MinCostFlow.Benchmarks
{
    public class SinglePassSolver
    {
        ...
        private readonly List<ProblemResult> _results = new();
        ...
        private List<MinCostFlowProblem> GatherAllProblems()
        {
            ...
            problems.Add(_repository.GenerateTransportationProblem(10, 10, 1000)); // 100 nodes
            problems.Add(_repository.GenerateTransportationProblem(22, 22, 1000)); // ~500 nodes
            problems.Add(_repository.GenerateTransportationProblem(31, 31, 1000)); // ~1000 nodes
            problems.Add(_repository.GenerateTransportationProblem(70, 70, 1000)); // ~5000 nodes
            problems.Add(_repository.GenerateTransportationProblem(100, 100, 1000)); // 10000 nodes
            ...
            // Add DIMACS problems
            problems.AddRange(StandardProblems.Dimacs.All());
            
            // Add small problems
            problems.Add(StandardProblems.Small.Path5Node);
            problems.Add(StandardProblems.Small.Path10Node);
            problems.Add(StandardProblems.Small.Grid2x2);
            problems.Add(StandardProblems.Small.DiamondGraph);
            
            // Sort by node count, then arc count
            return problems.OrderBy(p => p.NodeCount).ThenBy(p => p.ArcCount).ToList();
        }

        private ProblemResult SolveProblem(MinCostFlowProblem problem)
        {
